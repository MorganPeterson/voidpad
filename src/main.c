...
static const JanetAbstractType VPad = {
  "VoidPad",
  JANET_ATEND_NAME
};
...
cfun_make_void_pad(int32_t argc, Janet *argv) {
  ...
  if (strcmp(keyword, "string") == 0) {
    vp = create_string(janet_optcstring(argv, argc, 1, ""));
  } else if (strcmp(keyword, "number") == 0) {
    vp = create_empty_size(janet_optinteger(argv, argc, 1, 0));
...
cfun_vp_char_after_pointer(int32_t argc, Janet *argv) {
  ...
  char *result = char_after_pointer(vp, pointer);
  
  if (!result) {
    janet_panicf("pointer value not within text range: %d", pointer);
  } else {
...
cfun_vp_char_before_pointer(int32_t argc, Janet *argv) {
  ...
  char *result = char_before_pointer(vp, pointer);
